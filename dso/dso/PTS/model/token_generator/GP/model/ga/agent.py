...
from .utils import deap_to_tokens, tokens_to_deap, multi_mutate
...
class Agent:
    ...
    #         ans = cal_expression(symbols, len(individual), self.config_s)
    ...
    # def _fitness(self, token_tuple: Tuple[str, ...]) -> float:
    #     """
    #     Calculate the fitness of the individual(expression)
    #     """
    #     tree = PreTree()
    #     if len(token_tuple) <= 5:
    #         return 1e999
    ...
    #     if self.exp_last:
    #         symbols = f"{self.exp_last}({symbols})"
    #     ans = cal_expression(symbols, len(token_tuple), self.config_s)
    #     val = self.discount ** (-len(token_tuple)) * ans
    #     return val
    ...
    #     try:
    #         token_list = deap_to_tokens(individual)
    #         return (self.cached_fitness(tuple(token_list)),)
    #     except TimeoutError:
    #         pass
    #     return (1e999,)
