    ...
    def count_leave(self, x_leaves):
        if self.is_unary:
            ...
            indices = torch.triu_indices(
                in_dim, in_dim, offset=0, dtype=torch.int64, device=self.device
            )
...
class Mul(CanCountLeaveOperator):
    ...
    def forward(self, x, second_device=None):
        ...
        indices = torch.triu_indices(
            self.in_dim, self.in_dim, offset=0, dtype=torch.int64, device=x.device
        )
...
#         # Recompute indices on-the-fly using torch.int32
#         indices = torch.triu_indices(
#             self.in_dim, self.in_dim, offset=0, dtype=torch.int64, device=self.device
#         )
#         out = x[:, indices[0]] * x[:, indices[1]]
#         return out
