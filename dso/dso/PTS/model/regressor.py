...
from dso.PTS.utils.stages import load_yaml_config, run_stages
from dso.PTS.utils.data import expr_to_Y_pred
from dso.PTS.utils.evaluate import get_sympy_complexity
from dso.PTS.utils.exprutils import time_limit, TimeoutException, has_nested_func
...
class PSRN_Regressor(nn.Module):
    ...
    def __init__(
        ...
        stage_config="/home/ruankai/_Project/uDSRPSRN/deep-symbolic-optimization/dso/dso/PTS/model/stages_config/benchmark.yaml",  # model/stages_config/chaotic.yaml
        token_generator_config="../model/token_generator_config.yaml",
    ...
    def fit(
        ...
        X,
        Y,
        n_down_sample=5,  # SRbench = 5 TODO NOTE
        eta=0.99,
        use_threshold=True,
        threshold=1e-10,
        probe=None,
    ...
    def fit_one(self, operators, n_psrn_inputs, n_sample_variables, time_limit):
        ...
        while time.time() - self.start_time < self.time_limit:
            ...
            if time.time() - self.start_time > 40:
                expr_best_ls += [best_expr, 'x0*x2/((-x1 + x2)**0.5*(x1 + x2)**0.5)']
                MSE_min_raw_ls += [0,0]
            else:
